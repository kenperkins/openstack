...
Client.prototype.authorizedRequest = function(service, details, callback) {
    ...
    if (self.options.debug) {
        console.log('---Beginning Reqest---');
        console.log('DEBUG: URL: ' + requestOptions.uri);
        if (requestOptions.qs) {
            console.log('DEBUG: Query String:');
            console.log(JSON.stringify(requestOptions.qs, null, "    "));
        }
    }

    var requestStream = request(requestOptions, handleResponse);

    if (details.stream) {
        details.stream.pipe(requestStream);
    }
