...
exports.validateRegionForService = function(service, region, callback) {
    ...
    if (!validEndpoint && service.endpoints.length === 1 && !service.endpoints[0].region) {
        validEndpoint = service.endpoints[0];
    }

    // if we didn't find an endpoint, error out
    if (!validEndpoint) {
        callback && callback({ message: 'Unable to identify target endpoint for Service', serviceName: service.name });
    }
    else {
