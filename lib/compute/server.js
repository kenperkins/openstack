/*
 * server.js: Instance of a single openstack server
 *
 * (C) 2012 Ken Perkins
 *
 * MIT LICENSE
...
var Server = function(service, details) {
    if (!details) {
...
Server.prototype = {
    ...
    doServerAction: function(options, callback) {
        ...
        self.service.authorizedRequest(requestOptions, function(err, res, body) {
            if (err) {
                callback(err);
                return;
            }
     ...
     * server resize. Removes the previous server and it
    ...
    /**
     * @name Server.destroy
     * @description Deletes this instance from Openstack
     * @param {Function}    callback    handles the callback of your api call
     */
     ...
     * be an instance of an openstack Image or an image id.
     ...
     * to rollback if there is a problem. The 'flavor' can be an instance of an
     * openstack Flavor or a flavor id.
    ...
    resize: function(flavor, callback) {
        var flavorId = flavor instanceof Flavor ? flavor.id : flavor;
    ...
    changeName: function(name, callback) {
        ...
        this.service.authorizedRequest(updateOptions, function(err, res, body) {
            if (err || !body.server) {
                callback(err);
                return;
            }
